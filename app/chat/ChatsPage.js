...
export default class ChatsPage extends Component {
    ...
    componentDidMount(){
        if(ds.getRowCount()<=0){
            Alert.alert("初始化数据..."+ds.getRowCount());
...
const __renderSeparator = (sectionID:number,rowID:number) => {
    return (
        ...
        <View style={styles.row}>
            ...
            <View style={[styles.rightContainer]}>
                <View style={[{flexDirection:"row",flex:1,justifyContent:"space-between",alignItems:"center"}]}>
                    <Text style={styles.title} numberOfLines={1} >{chat.chattitle}</Text>
                    <Text style={styles.title}>{chat.lasttime}</Text>
                </View>
                <View style={[{flexDirection:"row",flex:1,justifyContent:"flex-start",alignItems:"center"}]}>
                    <Text style={[styles.title]} numberOfLines={1}>{chat.lastmessage}</Text>
                </View>
            </View>
        </View>
    );
